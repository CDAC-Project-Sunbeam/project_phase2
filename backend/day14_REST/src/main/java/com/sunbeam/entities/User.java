...
public class User extends BaseEntity {
	@Column( length = 25) // col name , varchar(25)
	...
	private byte[] image;
	// User 1---->1 Address
	@OneToOne(cascade = CascadeType.ALL, fetch = FetchType.LAZY) // def fetch type : EAGER , if needed change it to LAZY
	@JoinColumn(name = "address_id")
	private Address userAddress;
	// User 1--->1 AdhaarCard (uni dir only !) , association between entity n
	// composite value type
	// optional annotation
	@Embedded
	private AdhaarCard card;
	// User ---> Languages (String) => collection of basic value types
	@ElementCollection // mandatory
	// to customize name of collection table n it's FK column name
